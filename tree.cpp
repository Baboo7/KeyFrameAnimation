...
/*
 *  Initialise les attributs du TreeItem.
 */
...
TreeItem* BuildTree (vector<sprite_t *> *sprites_displaying)
{
    ...
    {
        cout << "ERREUR: Le fichier tree.txt est vide." << endl ;
        system("pause") ;
        exit(-1) ;
    }
...
void CalculateItemsPositionOnTreeRect (TreeItem *root, int X, int Y)
{
    int common_Y = Y ;
...
void GetDependances (TreeItem *item , string &s)
{
    for (unsigned int i=0 ; i<item->children.size() ; i++) s += item->name + ">" + item->children[i]->name + "\n" ;

    for (unsigned int i=0 ; i<item->children.size() ; i++) GetDependances (item->children[i], s) ;
}
...
void RotateWithChildren (TreeItem *item, sf::Vector2f rotation_center, double angle)
{
    sf::Vector2f original_g_c = item->m_sprite.sprite.getPosition() ;

    RotateVector(original_g_c, rotation_center, angle) ;
    RotateWithouthChildren(item, angle) ;
    item->m_sprite.sprite.setPosition(original_g_c) ;

    for (unsigned int i=0 ; i<item->children.size() ; i++)
    {
        RotateWithChildren(item->children[i], rotation_center, angle) ;
    }
}
